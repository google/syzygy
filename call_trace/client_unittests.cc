...
class TestCallTraceConsumer : public CallTraceEvents {
 public:
...
class CallTraceClientTest: public testing::PELibUnitTest {
 public:
  typedef testing::PELibUnitTest Super;

  CallTraceClientTest() : module_(NULL) {
  }
...
FARPROC CallTraceClientTest::_indirect_penter_dllmain_ = 0;
FARPROC CallTraceClientTest::_indirect_penter_ = 0;
...
void __declspec(naked) IndirectThunkDllMainImpl() {
  __asm {
    ...
    jmp CallTraceClientTest::_indirect_penter_dllmain_
  }
}
...
void __declspec(naked) IndirectThunkA() {
  __asm {
    ...
    jmp CallTraceClientTest::_indirect_penter_
  }
}
...
TEST_F(CallTraceClientTest, LoadUnload) {
...
TEST_F(CallTraceClientTest, NoServiceInstance) {
  ASSERT_NO_FATAL_FAILURE(LoadCallTraceDll());
...
TEST_F(CallTraceClientTest, NoSessionCreated) {
...
TEST_F(CallTraceClientTest, SingleThread) {
  ASSERT_NO_FATAL_FAILURE(StartCallTraceService(TRACE_FLAG_BATCH_ENTER));
...
TEST_F(CallTraceClientTest, MultiThreadWithDetach) {
  ASSERT_NO_FATAL_FAILURE(StartCallTraceService(TRACE_FLAG_BATCH_ENTER));
...
TEST_F(CallTraceClientTest, MultiThreadWithoutDetach) {
  ASSERT_NO_FATAL_FAILURE(StartCallTraceService(TRACE_FLAG_BATCH_ENTER));
...
TEST_F(CallTraceClientTest, RawCallSequence) {
  ASSERT_NO_FATAL_FAILURE(StartCallTraceService(TRACE_FLAG_BATCH_ENTER));
...
TEST_F(CallTraceClientTest, OrderedCallSequence) {
  ASSERT_NO_FATAL_FAILURE(StartCallTraceService(TRACE_FLAG_BATCH_ENTER));
...
TEST_F(CallTraceClientTest, MultiThreadWithStopCallTrace) {
  ASSERT_NO_FATAL_FAILURE(StartCallTraceService(TRACE_FLAG_BATCH_ENTER));
...
TEST_F(CallTraceClientTest, EnterExitRecursive) {
...
TEST_F(CallTraceClientTest, EnterExitTailRecursive) {
  ASSERT_NO_FATAL_FAILURE(
      StartCallTraceService(TRACE_FLAG_ENTER | TRACE_FLAG_EXIT));
...
TEST_F(CallTraceClientTest, EnterExitReturnAfterException) {
  top_entry = 0;
  top_exit = 0;
  bottom_entry = 0;
  bottom_exit = 0;
...
// entries on the shadow stack).
TEST_F(CallTraceClientTest, EnterExitCallAfterException) {
  top_entry = 0;
  top_exit = 0;
  bottom_entry = 0;
  bottom_exit = 0;
...
TEST_F(CallTraceClientTest, EnterExitCallAfterTailRecurseException) {
  top_entry = 0;
  top_exit = 0;
  bottom_entry = 0;
  bottom_exit = 0;
