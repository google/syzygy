...
#ifndef SYZYGY_CALL_TRACE_SHADOW_STACK_H_
#define SYZYGY_CALL_TRACE_SHADOW_STACK_H_
...
StackEntry& ShadowStackImpl<StackEntry>::Push(EntryFrame* frame) {
  ...
  DCHECK(stack_.empty() ||
      reinterpret_cast<const byte*>(stack_.back().entry_frame) >=
      reinterpret_cast<const byte*>(frame));
...
void ShadowStackImpl<StackEntry>::TrimOrphansOnExit(
  ...
  DCHECK(reinterpret_cast<const byte*>(stack_pointer) >
         reinterpret_cast<const byte*>(stack_.back().entry_frame))
...
#endif  // SYZYGY_CALL_TRACE_SHADOW_STACK_H_
