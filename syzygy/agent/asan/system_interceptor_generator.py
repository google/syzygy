...
_GENERATED_MESSAGE = """
%(c)s This file is generated by %(script)s, DO NOT MODIFY.
%(c)s This was generated with the following command-line:
%(c)s
%(c)s %(script)s
""" % {'script': os.path.basename(__file__), 'c': '%(c)s'}
for arg in sys.argv[1:]:
  _GENERATED_MESSAGE += "%(c)s    %(arg)s\n" % {'arg': arg, 'c': '%(c)s'}


_HEADER = """\
%(c)s Copyright %(year)d Google Inc. All Rights Reserved.
%(c)s
%(c)s Licensed under the Apache License, Version 2.0 (the "License");
%(c)s you may not use this file except in compliance with the License.
%(c)s You may obtain a copy of the License at
%(c)s
%(c)s     http://www.apache.org/licenses/LICENSE-2.0
%(c)s
%(c)s Unless required by applicable law or agreed to in writing, software
%(c)s distributed under the License is distributed on an "AS IS" BASIS,
%(c)s WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
%(c)s See the License for the specific language governing permissions and
%(c)s limitations under the License.
%(gen)s""" % {'year': datetime.datetime.now().year,
              'gen': _GENERATED_MESSAGE,
              'c': '%(c)s'}
...
_INSTRUMENTATION_FILTER_ENTRY_TEMPLATE = Template("""\
...
#         the intercepted function.
_INTERCEPTOR_TEMPLATE = Template("""
...
_PARAM_CHECKS_TEMPLATE = Template("""
...
class SystemInterceptorGenerator(object):
  ...
  def __init__(self, output_base, def_file, overwrite=False):
    ...
    self._output_impl_file = open(output_impl_filename, 'wb')
    self._output_impl_file.write(_HEADER % {'c': '//'})

    self._output_instrumentation_filter_file =  \
        open(output_instrumentation_filter_filename, 'wb')
    self._output_instrumentation_filter_file.write(_HEADER % {'c': '//'})
    ...
    with open(def_file, 'r') as f:
      ...
      self._def_file.write(template.format(
          r='check', s='rtl', message=_GENERATED_MESSAGE % {'c': ';'}))
      self._def_file.write('\n  ; Generated system intercepts\n')

      self._dyn_def_file.write(template.format(
          r='redirect', s='dyn', message=_GENERATED_MESSAGE % {'c': ';'}))
  ...
  def GenerateFunctionInterceptor(self, function_name, return_type,
    ...
    for m_iter in _ARG_TOKENS_RE.finditer(function_arguments):
      ...
      if m_iter.group('SAL_tag') in _TAGS_TO_CHECK_PRECALL:
        ...
        param_check_str = _PARAM_CHECKS_TEMPLATE.substitute(
    ...
    if m_buffer_size_arg:
      ...
      buffer_check = _PARAM_CHECKS_TEMPLATE.substitute(
    ...
    self._output_impl_file.write(_INTERCEPTOR_TEMPLATE.substitute(
    ...
    self._output_instrumentation_filter_file.write(
        _INSTRUMENTATION_FILTER_ENTRY_TEMPLATE.substitute(
            function_name=function_name,
...
def main(args):
  ...
  with SystemInterceptorGenerator(opts.output_base,
                                  opts.def_file,
