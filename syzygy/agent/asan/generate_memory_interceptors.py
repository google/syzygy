...
_CHECK_FUNCTION = """\
  ...
  ; in particular it saves and restores EFLAGS.
  ALIGN 16
  asan_check_{access_size}_byte_{access_mode_str} LABEL PROC  ; Probe \
#{probe_index}.
    {AsanSaveEflags}
    {AsanFastPath}
...
_CHECK_FUNCTION_NO_FLAGS = """\
  ...
  ; all other registers.
  asan_check_{access_size}_byte_{access_mode_str}_no_flags LABEL PROC  ; Probe \
#{probe_index}.
    {AsanFastPath}
    ; Restore original EDX.
    mov edx, DWORD PTR[esp + 4]
...
def _GenerateInterceptorsAsmFile():
  ...
  # Generate the header for the function housing all of the interceptors.
  parts.append(f.format(_INTERCEPTORS_PROC_HEADER))
  ...
  # Output the table of shadow references.
  ...
  parts.append(f.format(_INTERCEPTORS_PROC_TRAILER))
