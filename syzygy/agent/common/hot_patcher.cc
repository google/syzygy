...
bool HotPatcher::Patch(FunctionPointer function_entry_point,
  ...
  uint8* hot_patch_start = reinterpret_cast<uint8*>(function_entry_point) - 5;
  ...
  // entry point.
  int32* new_entry_point_place = reinterpret_cast<int32*>(hot_patch_start + 1);
  ...
  *new_entry_point_place =
      reinterpret_cast<uint8*>(new_entry_point) - hot_patch_start - 5;

  // This is the location where the short jump overwriting the first two bytes
  // of the function should be placed.
  volatile uint16* jump_hook_place =
      reinterpret_cast<uint16*>(hot_patch_start + 5);
