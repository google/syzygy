...
// Returns true if the the given references @p ref from @p referrer may not
// safely be redirected. If both the referrer and the referenced blocks are
// irregular in any way (i.e., inline assembly, not generated by cl.exe, etc)
// we cannot safely assume that @p ref has call semantics, i.e., where a return
// address is at the top of stack at entry. Ideally we would decide this on
// the basis of a full stack analysis, but beggars can't be choosers; plus, for
